# Configuring a Self-Signed Certificate for a Private Registry

If you need to interact with a private registry using a self-signed certificate, you can pass it as a PEM-encoded bundle when installing/upgrading the package. The Cartographer Conventions component will use it to establish a secure communication with the registry hosting the images referenced in a `PodIntent`.

```yaml
ca_cert_data: |
    -----BEGIN CERTIFICATE-----
    
    -----END CERTIFICATE-----
```

For more information, check the Cartographer Conventions documentation for [custom certificates](https://github.com/vmware-tanzu/cartographer-conventions/blob/main/README.md#install).
